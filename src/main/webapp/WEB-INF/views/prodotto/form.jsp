...
<form method="post" action="../prodotti/create" id="crmForm">

    <fieldset class="grid-x form admin">
        <legend>Crea Prodotto</legend>
        <label for="Nome">
            <input id="Nome" name="Nome" placeholder="Nome" type="text" value="${prodotto.nome}" pattern="^[a-zA-Z0-9_ .,']{2,50}$"
                   title="Nome compreso tra i 2 e 50 caratteri" required>
        </label>
        <label for="Prezzo" >
            <input id="Prezzo" name="Prezzo" placeholder="Prezzo" type="text" value="${prodotto.prezzo}" pattern="([0-9]*[.])?[0-9]+"
        ...
        <label for="Quantita" >
            <input id="Quantita" name="Quantita" placeholder="Quantita'" type="number" value="${prodotto.quantita}" pattern="^\\d+$"
                   title="La quantità deve essere un intero" required min="1">
        </label>
        <label for="Descrizione" >
            <input id="Descrizione" name="Descrizione" placeholder="Descrizione" type="text" value="${prodotto.descrizione}" pattern="^[a-zA-Z0-9_ .,)!séèò'’àùì…(™®:“”-]{2,250}$"
                   title="Descrizione compresa tra i 2 e 250 caratteri" required>
        </label>
        <label for="etaMinima" >
            <input id="etaMinima" name="etaMinima" placeholder="etaMinima" type="number" value="${prodotto.eta_minima}" pattern="^\\d+$" min="1"
                   required>
        </label>
